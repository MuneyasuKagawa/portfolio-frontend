...
// Elegant floating shapes for designer aesthetic
function FloatingShape({ delay = 0, duration = 6, className = "", index = 0 }) {
  const [isClient, setIsClient] = useState(false);
  const [dimensions, setDimensions] = useState({ width: 1200, height: 800 });

  // Create deterministic values for elegant movement
  const seedX1 = ((index * 31) % 100) / 100;
  const seedY1 = ((index * 67) % 100) / 100;
  const seedX2 = ((index * 97) % 100) / 100;
  const seedY2 = ((index * 139) % 100) / 100;
  const seedDuration = ((index * 19) % 15) / 10;
  ...
  if (!isClient) {
    return <div className={`absolute ${className}`} style={{ opacity: 0 }} />;
  }
    ...
    <span>
      ...
      {currentIndex < text.length && (
        <motion.span
          ...
          className="ml-0.5 inline-block h-6 w-0.5 bg-primary/60 align-middle"
        />
      )}
    </span>
  );
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        {[...Array(8)].map((_, i) => (
          <FloatingShape
            ...
            className={`${
              i % 4 === 0
                ? "h-32 w-32 rounded-full border border-primary/5 bg-gradient-to-br from-primary/5 to-transparent"
                : i % 4 === 1
                  ? "h-24 w-24 rounded-lg border border-purple-500/5 bg-gradient-to-tr from-purple-500/5 to-transparent"
                  : i % 4 === 2
                    ? "h-20 w-20 rounded-full border border-blue-500/5 bg-gradient-to-bl from-blue-500/5 to-transparent"
                    : "h-16 w-48 rounded-full border border-pink-500/5 bg-gradient-to-r from-pink-500/5 to-transparent"
            }`}
          />
        ))}
      </div>
      ...
      <div className="container relative z-10 flex flex-col items-center px-4 text-center">
        ...
        >
          ...
          >
            {isInView && <RevealText text="UI/UX Designer" delay={200} />}
          </motion.span>
        </motion.h1>
        ...
        >
          ...
          >
            I&apos;m a UI/UX designer with 8+ years of coding experience across
            front- and back-end.
          </motion.p>
          <motion.p
            className="mb-4"
          ...
          >
            Over the last six months, I have studied design at school and guided
            several team projects, from wireframes to clickable prototypes.
          </motion.p>
          <motion.p
            initial={{ opacity: 0 }}
            animate={isInView ? { opacity: 0.7 } : {}}
          ...
          >
            I aim to bridge design and development, helping teams launch
            products that look great, feel great for users, and stay easy to
            maintain for engineers.
          </motion.p>
        </motion.div>
